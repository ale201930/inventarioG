...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function addItemRow(prodIdDef = '', cantDef = 1, precioDef = '') {
    ...
    div.style.cssText = 'display:grid;grid-template-columns:2.2fr 1.4fr 0.7fr 1fr 1fr auto;gap:.5rem;margin-bottom:.4rem;';
      ...
      <select class="form-control item-prod" required>
        ...
        ${productos.map(p => `
          <option value="${p.id}" 
            ...
            data-pv1="${p.precio_venta1 || p.precio_unitario || 0}" 
            data-pv2="${p.precio_venta2 || 0}" 
            data-pv3="${p.precio_venta3 || 0}" 
            data-stock="${p.cantidad}"
            ${p.id === prodIdDef ? 'selected' : ''}>
            ${p.nombre} (Stock: ${p.cantidad})
          </option>`).join('')}
      </select>
      <select class="form-control item-nivel-precio" style="font-size:0.78rem; font-weight:600;">
        <option value="pv1">Precio 1 ($0.00)</option>
        <option value="pv2">Precio 2 ($0.00)</option>
        <option value="pv3">Precio 3 ($0.00)</option>
        <option value="custom">Personalizado</option>
      </select>
      <div style="position:relative;">
        <input type="number" class="form-control item-cant" placeholder="Cant." min="1" value="${cantDef}" required style="padding-right: 2rem;">
        <span class="stock-badge" style="position:absolute;right:3px;top:50%;transform:translateY(-50%);font-size:0.65rem;background:#e2e8f0;color:#475569;border-radius:3px;padding:1px 3px;pointer-events:none;">máx ?</span>
      </div>
      <input type="text" inputmode="decimal" class="form-control item-precio" placeholder="0.00" value="${precioDef}" required>
      <input type="text" class="form-control item-subtotal" placeholder="$0.00" readonly style="background:#f8fafc;font-weight:700;text-align:right;color:var(--success);">
    ...
    const selLevel = div.querySelector('.item-nivel-precio');
    ...
    function updatePriceOptions() {
      ...
      const pv1 = parseNum(opt.dataset.pv1);
      const pv2 = parseNum(opt.dataset.pv2);
      const pv3 = parseNum(opt.dataset.pv3);

      selLevel.options[0].textContent = `P1: $${pv1.toFixed(2)}`;
      selLevel.options[1].textContent = `P2: $${pv2.toFixed(2)}`;
      selLevel.options[2].textContent = `P3: $${pv3.toFixed(2)}`;

      const curLvl = selLevel.value;
      if (curLvl === 'pv1') inPrec.value = pv1.toFixed(2);
      else if (curLvl === 'pv2') inPrec.value = (pv2 > 0 ? pv2 : pv1).toFixed(2);
      else if (curLvl === 'pv3') inPrec.value = (pv3 > 0 ? pv3 : pv1).toFixed(2);
    }
    ...
    selLevel.addEventListener('change', () => {
      ...
      if (selLevel.value === 'pv1') inPrec.value = pv1.toFixed(2);
      if (selLevel.value === 'pv2') inPrec.value = (pv2 > 0 ? pv2 : pv1).toFixed(2);
      if (selLevel.value === 'pv3') inPrec.value = (pv3 > 0 ? pv3 : pv1).toFixed(2);
      calculateAllTotals();
    });

    inPrec.addEventListener('input', () => {
      selLevel.value = 'custom';
      calculateAllTotals();
    });
    ...
    if (sel.selectedIndex > 0) updatePriceOptions();
    calculateAllTotals();
  }
