...
async function ensureSchema() {
  ...
  try {
    await query('ALTER TABLE inventario ADD COLUMN precio_venta4 DECIMAL(10, 2) NOT NULL DEFAULT 0.00');
  } catch {
    // Si la columna ya existe, se ignora el error
  }
  schemaChecked = true;
}
...
export async function POST(request) {
  try {
    ...
    const pv4 = parseFloat(input.precioVenta4 ?? input.precio_venta4 ?? 0);
    ...
    // Upsert
    await query(
      `INSERT INTO inventario (id, nombre, cantidad, costo_unitario, precio_venta1, precio_venta2, precio_venta3, precio_venta4, precio_unitario, codigo_producto)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE nombre=VALUES(nombre), cantidad=VALUES(cantidad), costo_unitario=VALUES(costo_unitario),
       precio_venta1=VALUES(precio_venta1), precio_venta2=VALUES(precio_venta2), precio_venta3=VALUES(precio_venta3), precio_venta4=VALUES(precio_venta4),
       precio_unitario=VALUES(precio_unitario), codigo_producto=VALUES(codigo_producto)`,
      [id, nombre, cantidad, costo, pv1, pv2, pv3, pv4, pu, codigo]
    );
...
export async function PUT(request) {
  try {
    ...
    await query(
      `UPDATE inventario SET nombre=?, cantidad=?, costo_unitario=?, precio_venta1=?, precio_venta2=?, precio_venta3=?, precio_venta4=?, precio_unitario=? WHERE id=?`,
      [input.nombre.trim(), parseInt(input.cantidad ?? 0), costo, pv1, pv2, pv3, pv4, pu, input.id]
    );
