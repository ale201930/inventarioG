    ...
    if (isAndroid) {
      try {
        ...
        if (h2c) {
          ...
          printDiv.style.padding = '8px 4px';
          printDiv.style.fontFamily = 'Arial, Helvetica, sans-serif';
          printDiv.style.boxSizing = 'border-box';
          printDiv.style.lineHeight = '1.35';

          printDiv.innerHTML = `
            <div style="text-align:center; font-weight:800; font-size:26px; margin-bottom:2px;">BESTEDA 2, C.A.</div>
            <div style="text-align:center; font-weight:700; font-size:16.5px; margin:2px 0;">RIF: J-40529263-6</div>
            <div style="text-align:center; font-size:15px; margin:1px 0;">Calle Principal Casa Nº A-13, Urb. Alto de Fenix II</div>
            <div style="text-align:center; font-size:15px; margin:1px 0;">San Juan de los Morros - Estado Guárico</div>
            <div style="text-align:center; font-size:15px; margin:1px 0;">Tlfs: 0424-313.68.05 / 0424-300.48.02</div>
            
            <hr style="border:none; border-top:2.5px solid #000; margin:10px 0;" />
            
            <div style="text-align:center; font-weight:800; font-size:22px; letter-spacing:0.5px;">NOTA DE ENTREGA</div>
            <div style="text-align:center; font-weight:800; font-size:22px; margin-top:2px;">Nº ${lastSalida.factura_number}</div>
            
            <hr style="border:none; border-top:2px dashed #000; margin:10px 0;" />
            
            <div style="display:flex; justify-content:space-between; font-size:17px; padding:3px 0;"><b>FECHA:</b><span>${cleanFecha}</span></div>
            <div style="display:flex; justify-content:space-between; font-size:17px; padding:3px 0;"><b>CLIENTE:</b><span style="font-weight:700;">${lastSalida.cliente_name || ''}</span></div>
            <div style="display:flex; justify-content:space-between; font-size:17px; padding:3px 0;"><b>C.I./RIF:</b><span>${lastSalida.cedula_rif || '—'}</span></div>
            <div style="display:flex; justify-content:space-between; font-size:17px; padding:3px 0;"><b>TELF:</b><span>${lastSalida.telefono || '—'}</span></div>
            <div style="display:flex; justify-content:space-between; font-size:17px; padding:3px 0;"><b>DIR:</b><span>${lastSalida.direccion || '—'}</span></div>
            
            <hr style="border:none; border-top:2px dashed #000; margin:10px 0;" />
            
            <table style="width:100%; border-collapse:collapse; font-size:17px; margin:10px 0; table-layout:fixed;">
              <thead>
                <tr style="border-bottom:2.5px solid #000;">
                  <th style="text-align:left; width:12%; padding:5px 0; font-size:16.5px; font-weight:800;">CAN</th>
                  <th style="text-align:left; width:46%; padding:5px 0; font-size:16.5px; font-weight:800;">DESCRIPCIÓN</th>
                  <th style="text-align:right; width:21%; padding:5px 0; font-size:16.5px; font-weight:800;">P/U</th>
                  <th style="text-align:right; width:21%; padding:5px 0; font-size:16.5px; font-weight:800;">TOTAL</th>
                </tr>
              </thead>
              <tbody>
                ${items.map(it => {
                  ...
                  return `
                    <tr style="border-bottom:1px dashed #000;">
                      <td style="padding:7px 0; vertical-align:top; font-size:17px; font-weight:700;">${cant}</td>
                      <td style="padding:7px 0; vertical-align:top; font-size:17px; font-weight:700; word-break:break-word;">${it.productoNombre || it.producto_nombre}</td>
                      <td style="text-align:right; padding:7px 0; vertical-align:top; font-size:17px;">$${pu.toFixed(2)}</td>
                      <td style="text-align:right; padding:7px 0; vertical-align:top; font-size:17px; font-weight:800;">$${tot.toFixed(2)}</td>
                    </tr>
                  `;
                }).join('')}
              </tbody>
            ...
            <div style="display:flex; justify-content:space-between; font-weight:800; font-size:22px; margin:14px 0;">
              <span>UND: ${totalUnits}</span>
              <span>TOTAL: $${Number(lastSalida.total_factura || 0).toFixed(2)}</span>
            </div>
            
            <div style="border:2px solid #000; border-radius:8px; padding:12px 14px; margin:14px 0 8px 0; background:#fff; font-size:15px; line-height:1.45; color:#000;">
              <div style="font-weight:800; font-size:16px; text-align:center; margin-bottom:8px;">— PAGO MÓVIL BDV —</div>
              ...
              <div style="font-weight:800; font-size:16px; text-align:center; margin-bottom:8px;">— DEPÓSITO BANCARIO BDV —</div>
              <div style="display:flex; flex-direction:column; gap:4px;">
                <div>• <strong>0102 0467 4501 0162 8166</strong> <span style="font-size:13px;">(JUAN MORA)</span></div>
                <div>• <strong>0102 0467 4500 0096 7787</strong> <span style="font-size:13px;">(JORGE FLORES)</span></div>
              </div>
            </div>
          `;
