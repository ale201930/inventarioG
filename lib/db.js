// lib/db.js - Conexión MySQL2 a TiDB Cloud con Pool Reutilizable y Sanitización de Variables
...
export function getPool() {
  ...
  const user = clean(process.env.TIDB_USER || process.env.DB_USER, '3SA3TXmBlV6gNHj.admin');
  const password = clean(process.env.TIDB_PASSWORD !== undefined ? process.env.TIDB_PASSWORD : process.env.DB_PASS, 'Inventario2026!');
  ...
  if (process.env.DATABASE_URL) {
    ...
    const sslParam = cleanUrl.includes('tidbcloud') ? '?ssl={"rejectUnauthorized":false}' : '';
